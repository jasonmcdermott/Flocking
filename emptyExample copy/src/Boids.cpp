...
Boids::Boids(int iNumBoids) {
	...
	for (int i = 0; i < num_boids; ++i) {
		...
		boids.back()->setPosition(ofVec3f(ofRandom(0,ofGetWidth()),ofRandom(0,ofGetHeight()),ofRandom(0,800)));
//		boids.back()->setPosition(ofVec3f(ofGetWidth()/2,ofGetWidth()/2,400));
	}
}
...
void Boids::seekParent(ofVec3f oMainTarget) {
 ...
 */
void Boids::flock() {
	float too_close_dist = 30.0f;
	float in_sight_dist = 100.0f;
	int insight_count = 0;
...
void Boids::arrive(ofVec3f oTarget) {
	for (int i = 0; i < boids.size(); ++i) {
		boids[i]->arrive(oTarget);
	}
}
void Boids::flee(ofVec3f oTarget) {
	for (int i = 0; i < boids.size(); ++i) {
...
void Boids::explode() {
	...
	for (int i = 0; i < boids.size(); ++i) {
		boids[i]->setVelocity(ofVec3f(ofRandom(-max_vel*10,max_vel*10),ofRandom(-max_vel*10,max_vel*10),ofRandom(0,-max_vel*10)));
	}
}
